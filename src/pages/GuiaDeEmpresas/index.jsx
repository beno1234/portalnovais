...
const GuiaDeEmpresas = () => {
  ...
  const onLocationSelect2 = (id) => {
    setSelectedLocation(id);
    searchParams.set("location", id.toString());
    setSearchParams(searchParams);
  };

  const {
    list: listCategory,
    indexOrder: indexOrderCategory,
    isLoading: isLoadingCategory /* , error: errorCategory */,
  } = useSelector((state) => state.category);
  const {
    list: listLocation,
    indexOrder: indexOrderLocation,
    isLoading: isLoadingLocation /* , error: errorLocation */,
  ...
  const onCategorySelect = (id) => {
    ...
    selectedCategories.current.length > 0
      ? searchParams.set("category", selectedCategories.current.toString())
  ...
  const onLocationSelect = (id) => {
    selectedLocations.current.includes(id)
      ? (selectedLocations.current = selectedLocations.current.filter(
          (n) => n !== id
        ))
      : selectedLocations.current.push(id);

    selectedLocations.current.length > 0
      ? searchParams.set("location", selectedLocations.current.toString())
      : searchParams.delete("location");
    setSearchParams(searchParams);
  };

  const certifiedSelection = useRef(
    searchParams.has("type") && searchParams.get("type") === "certified"
      ? true
      : false
  );
  ...
  const onCertifiedSelect = () => {
    ...
    certifiedSelection.current
      ? searchParams.set("type", "certified")
            ...
            <div className="ui equal width stackable grid">
              <div className="center aligned row">
                <div className="column">
                  <h1 className="ui inverted header">Guia de Empresas</h1>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="divGeral">
                    ...
                    <div className="inline field">
                      <div className="ui checkbox">
                        <input
                          ...
                          onChange={onCertifiedSelect}
                          checked={certifiedSelection.current}
                        />
                        <label htmlFor="certifiedCheckbox">
                          Apenas anúncios certificados
                        </label>
                      </div>
                    </div>
                  </div>
                        ...
                        <div className="inline field">
                          <div className="ui checkbox">
                            <input
                              ...
                              checked={selectedCategories.current.includes(
                                category.id
                              )}
                            />
                ...
                <div className="ui stackable two column grid">
                  {locations.length > 0 ? (
                    locations.map((location) => (
                      <div className="option column" key={location.name}>
                        <div className="inline field">
                          <div className="ui radio checkbox">
                            <input
                              ...
                              className="hidden"
                              id={location.name}
                              onChange={() => onLocationSelect2(location.id)}
            ...
            <div className="divPubli">
              ...
              <div className="ui container">
                ...
                <Publicidade
                  className="publi"
                  types={["featured", "miniBanner"]} // Tipos diferentes para os dois banners laterais
                  category={searchParams.get("category")}
                  locations={selectedLocation ? [selectedLocation] : []}
                />
