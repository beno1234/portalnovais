...
const PublicAdvertisementList = ({ searchParams }) => {
    ...
    const { list, totalCount, indexOrder, isLoading } = useSelector((state) => state.advertisement);
    ...
    return <>
        {
            (advertisements.length > 0 && !isLoading)
                ? <>
                    ...
                    {
                        advertisements.map((advertisement, index) => <div className="eight wide tablet five wide computer column" key={advertisement.name + index}>
