...
import { fetchAdvertisement, incrementAdvertisementView, incrementAdvertisementWhatsappView, rateAdvertisement } from '../../store/actions/advertisement';
                    ...
                    <div className='ui stackable centered grid'>
                        ...
                        {!error
                            ? advertisement && <>
                                <div className='ten wide column'>
                                    <div className='ui very padded segment'>
                                                ...
                                                <div className='ui large middle aligned feature list'>
                                                    <div className='item'>
                                                        ...
                                                        <div className='middle aligned content'>
                                                            <div className='middle aligned content'>
                                                                <b>{Math.round(advertisement.rating * 10) / 10}</b>/5 de avaliação
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div className='item'>
                                        ...
                                        <Carousel>
                                            <div>
                                                <img alt="Capa" src={image} style={{ maxHeight: "800px" }} />
                                            </div>
                                            {
                                                imagesCarousel.map((image, index) => {
                                                    return <div key={`carouselImage${index}`}>
                                                        <img alt={index} src={image} style={{ maxHeight: "800px" }} />
                                                    </div>
                                                })
                                            }
                                        </Carousel>
                                        ...
                                        {isSignedIn
                                            ? <>
                                                <div className='field'>
                                                    <p style={{ fontSize: "1.3em" }}>Avalie o serviço!</p>
                                                </div>
                                                <Rating icon='star' maxRating={5} onRate={onRateClick} defaultRating={Math.floor(advertisement.rating)} />
                                            </>
                                            : <div className="ui warning message">Você precisa fazer login para avaliar esse anunciante.</div>
                                        }

                                        {advertisement.type === "certified" && <>
                                            <div className="ui basic segment" style={{ marginTop: "5em" }}>
                                                <div className="ui center aligned icon header">CERTIFICADO
                                        ...
                                        {advertisement.sale && <>
                                            <div className="ui raised segment" style={{ marginBottom: "5em" }}>
                                                <div className="ui orange ribbon label">PROMOÇÃO</div>
                                        ...
                                        {
                                            Object.values(advertisement.categories).map((item, index) => <div className="ui tag category label" key={`infoCategory${index}`}>{item.category.name}</div>)
                                        }

                                        <div className='ui basic secondary segment'>
                                            <div className='ui big middle aligned feature list'>

                                                {advertisement.phone && <div className='item'>
                                                ...
                                                {advertisement.whatsapp && <div className='item'>
                                                    <i className='whatsapp alternate golden icon'></i>
                                                    <div className='content'>
                                                        <p>
                                                            <a href={`https://web.whatsapp.com/send?phone=55${advertisement.whatsapp.replace(/\D/g, "")}`} target="_blank" rel="noopener noreferrer" className="ui green right floated button" onClick={() => onWhatsAppClick(advertisement?.id)}>
                                                ...
                                                {advertisement.locations && <div className='item'>
                                                    <i className='location arrow golden icon'></i>
                                                    <div className='content'>
                                                        <p>
                                                            <b>Região</b><br />
                                                            {
                                                                advertisement.locations && Object.values(advertisement.locations).map((item, index) => <span style={{ marginRight: "1em" }} key={`infoLocation${index}`}>{item.location.name}</span>)
                                                            }
                                                        </p>
                                                    </div>
                                                </div>}
                                                ...
                                                {advertisement.email && <div className='item'>
                                                ...
                                                {advertisement.site && <div className='item'>
                                                    <i className='globe golden icon'></i>
                                                    <div className='content'>
                                                        <p>
                                                            <b>Website</b>{' '}
                                                            <a href={advertisement.site}>{advertisement.site}</a>
                                                        </p>
                                                    </div>
                                                </div>}

                                                {advertisement.facebook && <div className='item'>
                                                    <i className='facebook golden icon'></i>
                                                    <div className='content'>
                                                        <p>
                                                            <b>Facebook</b>{' '}
                                                            <a href={advertisement.facebook}>{advertisement.facebook}</a>
                                                        </p>
                                                    </div>
                                                </div>}

                                                {advertisement.instagram && <div className='item'>
                                                    <i className='instagram golden icon'></i>
                                                    <div className='content'>
                                                        <p>
                                                            <b>Instagram</b>{' '}
                                                            <a href={advertisement.instagram}>{advertisement.instagram}</a>
                                                        </p>
                                                    </div>
                                                </div>}
                                            </div>
                                        ...
                                        <RichTextEditor
                                            editorState={
                                                isJsonString(advertisement?.description)
                                                    ? EditorState.createWithContent(convertFromRaw(JSON.parse(advertisement?.description)))
                            ...
                            : <div className="ui error message">
                                ...
                                <button className='ui button' type="button" onClick={() => onFetchAdvertisement(id)}>Carregar os dados</button>
                            </div>}
                    </div>
                </div>
