...
export const setError = (err) => {
    if (!!err) {
        if (err.message === "Unauthorized") err.message = "E-mail ou senha incorretos.";
        switch (err.code) {
            case 401:
...
// Enva o codigo no email
export const requestResetPassword = async (formValues) => {
    try {
        await api.patch("/users/request-reset-password", formValues);
        return true;
    } catch (err) {
        return false;
    }
...
export const resetPassword = async (formValues) => {
    try {
        await api.patch("/users/reset-password", formValues);
